...
const Registration = () => {
    ...
    const validateCreditCards = () => {
        // Check if any credit card number is filled, and if so, validate it
        ...
        const invalidExpiry = creditCards.some(card =>
            card.number && (!card?.expiry || !/^((0[1-9])|(1[0-2]))\/(\d{2})$/.test(card.expiry))
        );
        if (invalidExpiry) {
    ...
    const handleSubmit = async (e) => {
        ...
        // Validate credit cards if filled
        if (!validateCreditCards()) {
            return;
        }
        ...
        const userData = {
            ...
            postalCode, // Add postal code
            state, // Add state
            creditCardNumber: creditCards.filter(card => card.number).map(card => card.number), // Only include cards with numbers
            expiryDate: creditCards.filter(card => card.expiry).map(card => card.expiry), // Only include cards with expiry dates
            cvv: creditCards.filter(card => card.cvv).map(card => card.cvv), // Only include cards with CVVs
            promotionOptIn,
        };
            ...
            <div className="w-full max-w-lg">
                <div className="bg-white py-8 px-4 shadow rounded-lg sm:px-10">
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <h3 className="text-lg font-semibold mt-4">Credit Cards (Optional)</h3>
                        ...
                        {creditCards.map((card, index) => (
                            <div key={index} className="mt-2">
                                ...
                                <label className="block text-sm font-medium text-gray-700" htmlFor={`expiry${index}`}>
                                    Expiry Date (MM/YY)
                                </label>
                                <input
                                    id={`expiry${index}`}
                                ...
                                <label className="block text-sm font-medium text-gray-700" htmlFor={`cvv${index}`}>
                                    CVV
                                </label>
                                <input
                                    id={`cvv${index}`}
                        ...
                        <div className="mt-4">
                            <label className="inline-flex items-center text-sm font-medium text-gray-700" htmlFor="promotionOptIn">
                                <input
                                    id="promotionOptIn"
                                    type="checkbox"
                                    checked={promotionOptIn}
                                ...
                                />
                                <span className="ml-2">Register for promotions and offers</span>
                            </label>
                        </div>
                        ...
                        <div className="mt-6">
                            <button
                                type="submit"
                                className="w-full px-4 py-2 bg-blue-600 text-white font-semibold text-sm rounded-lg shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                                Register
                            </button>
                        </div>
